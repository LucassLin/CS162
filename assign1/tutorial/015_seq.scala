...
def lengthThree(input: List[String]): List[String] = input match {
  case Nil => List()
...
assert(Seq(1, 2, 3).foldLeft(List[Int]())((list, num) => num :: list) == List(3, 2, 1)) // don't use reverse

// hint: Strings have a `length` method
assert(Seq("moo", "cow", "bull").foldRight(List[Int]())((str, list) => str.length :: list) == List(3, 3, 4))
