...
somethingRandom match {
  ...
  case OtherBase(Foo(x)) => println("Found an OtherBase holding a Foo")
  case OtherBase(Bar(7,y)) => println("Found an OtherBase holding a Bar, which " +
                      "itself was holding 7")
  case b@Bar(x,y) => takesBar(b)
