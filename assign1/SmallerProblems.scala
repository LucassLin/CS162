...
object Problem2 {
  ...
  def order( node:Node ) {
    ...
    def preorder(node: Option[Node], id: Int) : Int = {
      node match {
          case Some(node) =>
            ...
            preorder(node.right, preorder(node.left, id + 1))
          case None => id 
      }
    }
    def postorder(node:Option[Node], id:Int) : Int = {
      node match {
          case Some(node) =>
            val nextId = postorder(node.right, postorder(node.left, id))
            node.ord = (node.ord._1 -> nextId)
            nextId + 1
          case None => id
    }
  }
...
object Problem3 {
  def main( args:Array[String] ) = {
    ...
    println(list.foldLeft(List[String]())((list, str) => if (list.contains(str)) list else str :: list).reverse.toString)
    println(list.foldLeft(List[(String, Int)]())((newList, str) => 
      newList match{
        case (word, count)::tail =>
          if (word == str) (word, count + 1) :: tail
          else (str,1) :: newList
        case Nil => (str, 1) :: newList
        }
        ).reverse)
